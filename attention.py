...
def create_causal_mask(embed_dim, n_heads, max_context_len):
    """
        ...
        """
    """
    # Create a lower triangular matrix with ones below the diagonal and zeros elsewhere
    mask = torch.tril(torch.ones((1, max_context_len, max_context_len), dtype=torch.float32))

    # Reshape to ensure it matches the dimensions required for multi-head attention
